...
//  config.map(TweetProducer(client).publish(_))
  ...
  val ip32bits = java.lang.Long.parseLong(ip.split("\\.").map(s => ("00000000" + (s.toInt.toBinaryString)).substring(s.toInt.toBinaryString.length)).mkString, 2)

  val mask32bits = java.lang.Long.parseLong(mask.split("\\.").map(s => ("00000000" + (s.toInt.toBinaryString)).substring(s.toInt.toBinaryString.length)).mkString, 2)

  println((ip32bits & mask32bits).toBinaryString.toCharArray.grouped(8).map(_.mkString).map(Integer.parseInt(_, 2)).mkString("."))
